...
// Exact-match repairs for poles a feed names badly. The TTC needs none: the
// names are the street pairs the poles carry ("Bathurst St at Horsham Ave"),
// stations are "Kipling Station", bus bays "Don Mills Station at Bus Bay 2".
const NAME_FIX = {};
...
// BRT, Cairo's paratransit) — unused in Toronto, kept so the engine code
...
// its own map: yellow 1 Yonge-University, green 2 Bloor-Danforth, purple 4
// Sheppard, orange 5 Eglinton, grey 6 Finch West. The streetcars keep the
// family red and the buses the family navy: the feed paints the expresses
// green and the Blue Night routes blue, but colour means the MODE here.
const ALL_ROUTES = await readCsv(join(ROOT, 'data/gtfs/routes.txt'));
const RAPID = new Set(['1', '2', '4', '5', '6']);
const TOR_COLORS = {};
...
// light rail platforms are one stop_id each and named "Bloor Station -
// Southbound Platform" — the station is the name on the sign, so the platform
// tail goes and the two platforms merge into one station disc. The HEADSIGNS
// carry the whole announcement ("East - 10 Van Horne towards Victoria Park",
// "Yonge-University Line towards Finch Station") and the panel prints them as
// directions, so everything up to "towards" goes — the short-turn signs
// without a "towards" stay as they are.
const fixName = (n) => n.replace(/\s+/g, ' ').trim()
  .replace(/\s+-\s+(?:North|South|East|West)bound Platform$/i, '')
  .replace(/^.*\btowards\s+/i, '');
...
if (tramAll || tramSel.length) MODES.push({
  // the ten streetcar routes and the six Blue Night streetcars, all on
  // railway=tram — Spadina, St Clair and the Harbourfront on reserved track
  mode: 'tram', label: 'streetcars', osmFile: 'data/osm/toronto-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram']),
  // Toronto's mappers tag service=siding on running track too — curves and
  // whole blocks of King and Queen carry it — and on the connectors the TTC
  // keeps for diversions (Church, Parliament, Bathurst north of College,
  // noted "Non-revenue trackage"). The family rule (tram sidings are depot
  // tracks, out of the graph) tore 306/506 and 503 into raw stretches: the
  // 506's current shape leaves Carlton at Church and comes back down
  // Parliament, on exactly those connectors. Every siding is therefore a
  // track here; the depots stay out as service=yard.
  retag: (e) => {
    if (e.tags?.railway === 'tram' && e.tags.service === 'siding') delete e.tags.service;
  },
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel,
  feeds: [
    { tag: 'ttc', dir: 'data/gtfs', routeTypes: ['0'],
      skipRoute: (r) => RAPID.has((r.route_short_name || '').trim()),
      mapKey: (sn) => sn || null, nameFix: fixName, noFeedColors: true },
  ],
});
if (tramAll || rapidSel.length) MODES.push({
  // subway lines 1, 2 and 4 on railway=subway, the light rail lines 5 and 6
  // on railway=light_rail; allMetro gives every line the trunk treatment
  // without an M-prefix to key off. No allVariants: none of the five lines
  // has a branch.
  mode: 'tram', label: 'rapid transit', osmFile: 'data/osm/toronto-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway', 'light_rail']),
  allMetro: true,
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : rapidSel,
  feeds: [
...
await (await import('./night.mjs')).nightPass(outDir, /^3\d\d$/, { sort: false });
